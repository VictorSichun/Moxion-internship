 ...
 */
    const all = function (fn1) {
        return function (arr) {
        return arr.reduce((b, x) => {
     return b && Boolean(fn1(x))
}, true)
}
}
    let any = function (fn1) {
    return function (arr) {
        return arr.reduce((b, x) => {
     return b || Boolean(fn1(x))
}, false)
 ...
 */ /** */
    ...
    let isUndefined = tis('undefined')
    let isDefined = not(isUndefined)
    let isNull = function (o) {
    return o === null
}
    let isNullOrUndefined = or(isNull, isUndefined)
    let isFunction = tis('function')
    let isNumber = tis('number')
    let isString = tis('string')
 ...
 */
    ...
    let notImplemented = function (fnname) {
    return function () {
 ...
 */ /** for typedoc */
    ...
    let angular$1 = w.angular || {}
    let fromJson = angular$1.fromJson || JSON.parse.bind(JSON)
    let toJson = angular$1.toJson || JSON.stringify.bind(JSON)
    let copy = angular$1.copy || _copy
    let forEach = angular$1.forEach || _forEach
    let extend = angular$1.extend || _extend
    let equals = angular$1.equals || _equals
 ...
 */
    ...
    let makeLateRebindFn = function (fnName) {
 ...
 *        pick(obj, ["foo", "bar"]); // returns { foo: 1, bar: 2 }
 */
    let restArgs = function (args, idx) {
        if (idx === void 0) {
        idx = 0
}
 ...
 */
    function defaults(opts) {
        ...
        const defaults = merge.apply(null, [{}].concat(defaultsList))
...
/** Return a copy of the object omitting the blacklisted properties. */
    function omit(obj) {
        const notInArray = function (array, item) {
 ...
 * ```
 */
    let allTrueR = function (memo, elem) {
        return memo && elem
}
/**
 ...
 * ```
 */
    let anyTrueR = function (memo, elem) {
        return memo || elem
}
/**
 ...
 * ```
 */
    let unnestR = function (memo, elem) {
 ...
 */
    function arrayTuples() {
        ...
        {
return []}
        const length = arrayArgs.reduce((min, arr) => {
        return Math.min(arr.length, min)
}, 9007199254740991) // aka 2^53 âˆ’ 1 aka Number.MAX_SAFE_INTEGER
        return Array.apply(null, Array(length)).map((ignored, idx) => {
        return arrayArgs.map(arr => {
     return arr[idx]
})
})
    }
 ...
 */
    function _copy(src, dest) {
        if (dest)        {Object.keys(dest).forEach((key) => {
...
return true} // meh
        ...
        if (predicates.map(any).reduce((b, fn) => {
        return b || Boolean(fn(tup))
}, false))
        {
return false}
        let key,
    ...
    function _arraysEq(a1, a2) {
        ...
        return arrayTuples(a1, a2).reduce((b, t) => {
 ...
 */
    const sortBy = function (propFn, checkFn) {
        if (checkFn === void 0) {
        checkFn = val(true)
}
        return function (a, b) {
 ...
 */
    let composeSort = function () {
    const sortFns = []
    for (let _i = 0; _i < arguments.length; _i++) {
        sortFns[_i] = arguments[_i]
    }
    return function (a, b) {
        return sortFns.reduce((prev, fn) => {
            return prev || fn(a, b)
}, 0)
    }
}
// issue #2676
    let silenceUncaughtInPromise = function (promise) {
    return promise.catch(e => {
        return 0
}) && promise
}
    let silentRejection = function (error) {
 ...
 */ /** for typedoc */
    ...
    Rejection.superseded = function (detail, options) {
        ...
        let rejection = new Rejection(exports.RejectType.SUPERSEDED, message, detail)
 ...
 */
    ...
    Trace.prototype._set = function (enabled, categories) {
        ...
        categories.map(normalizedCat).forEach(category => {
...
return}
    ...
    TransitionHook.runOnBeforeHooks = function (hooks) {
        const results = []
        for (let _i = 0, hooks_1 = hooks; _i < hooks_1.length; _i++) {
            const hook = hooks_1[_i]
...
return nodes}
    ...
    RegisteredHook.prototype._getDefaultMatchCriteria = function () {
        return map(this.tranSvc._pluginapi._getPathTypes(), () => {
            return true
})
    }
    /**
    ...
    RegisteredHook.prototype._getMatchingNodes = function (treeChanges) {
        ...
        let paths = values(this.tranSvc._pluginapi._getPathTypes())
        return paths.reduce((mn, pathtype) => {
            ...
            let nodes = isStateHook ? path : [tail(path)]
...
/** @hidden Return a registration function of the requested type. */
    ...
    // Create hook registration function on the IHookRegistry for the event
        ...
        const registeredHook = new RegisteredHook(transitionService, eventType, callback, matchObject, options)
 ...
 */
    ...
    HookBuilder.prototype.buildHooks = function (hookType) {
        ...
        const makeTransitionHooks = function (hook) {
            ...
            const matches = hook.matches(_this.treeChanges)
            // Select the PathNode[] that will be used as TransitionHook context objects
            let matchingNodes = matches[hookType.criteriaMatchPath.name]
            ...
            return matchingNodes.map(node => {
                ...
                let state = hookType.criteriaMatchPath.scope === exports.TransitionHookScope.STATE ? node.state : null
                let transitionHook = new TransitionHook(_this.transition, state, hook, _options)
...
return val}
            ...
            return (allTruthyMode === true) ? filter(result, x => {
                return !x
...
return false}
        ...
        let wrapperFn = name === 'equals' ? arrayEqualsHandler : arrayHandler
...
replace.push({from: squash, to: undefined})}
        ...
        return filter(defaultPolicy, item => {
    ...
    const Param = (function () {
        ...
        let isOptional = config.value !== undefined || location === exports.DefType.SEARCH
        let dynamic = isDefined(config.dynamic) ? Boolean(config.dynamic) : Boolean(type.dynamic)
        let raw = isDefined(config.raw) ? Boolean(config.raw) : Boolean(type.raw)
        ...
        let inherit$$1 = isDefined(config.inherit) ? Boolean(config.inherit) : Boolean(type.inherit)
...
throw new Error('Default value (' + defaultValue + ') for parameter \'' + _this.id + '\' is not an instance of ParamType (' + _this.type.name + ')')}
        ...
        let $replace = function (val$$1) {
            const replacement = map(filter(_this.replace, propEq('from', val$$1)), prop('to'))
 ...
 */
    let PathNode = (function () {
        ...
        if (stateOrPath instanceof PathNode) {
            ...
            this.paramValues = {}
            this.resolvables = state.resolvables.map(res => {
                return res.clone()
})
        }
    }
    /** Sets [[paramValues]] for the node, from the values of an object hash */
        ...
        this.paramValues = this.paramSchema.reduce((memo, pDef) => {
    ...
    PathNode.prototype.equals = function (node, keys) {
        const _this = this
        if (keys === void 0) {
            keys = this.paramSchema.map(p => {
     return p.id
})
}
        const paramValsEq = function (key) {
    ...
    PathNode.matching = function (pathA, pathB, ignoreDynamicParams) {
        ...
        for (let i = 0; i < pathA.length && i < pathB.length; i++) {
            ...
            const changedParams = Param.changed(a.paramSchema, a.paramValues, b.paramValues)
                .filter(param => {
 ...
 */
    ...
    PathFactory.buildPath = function (targetState) {
        ...
        return targetState.$state().path.map(state => {
            return new PathNode(state).applyRawParams(toParams)
})
    }
    /** Given a fromPath: PathNode[] and a TargetState, builds a toPath: PathNode[] */
    PathFactory.buildToPath = function (fromPath, targetState) {
        const toPath = PathFactory.buildPath(targetState)
    ...
    PathFactory.applyViewConfigs = function ($view, path, states) {
     ...
     let subPath = PathFactory.subPath(path, n => {
                return n === node
})
     let viewConfigs = viewDecls.map(view => {
    ...
    PathFactory.inheritParams = function (fromPath, toPath, toKeys) {
        ...
        const noInherit = fromPath.map(node => {
...
throw new Error('new Resolvable(): resolveFn argument must be a function')}
    ...
    Resolvable.prototype.resolve = function (resolveContext, trans) {
        ...
        let getResolvableDependencies = function () {
            return $q.all(resolveContext.getDependencies(_this).map(r => {
                return r.get(resolveContext, trans)
            }))
        }
        // Invokes the resolve function passing the resolved dependencies as arguments
        let invokeResolveFn = function (resolvedDeps) {
...
/** @module resolve */
/** for typedoc */
    let when = resolvePolicies.when
    let ALL_WHENS = [when.EAGER, when.LAZY]
    let EAGER_WHENS = [when.EAGER]
    let NATIVE_INJECTOR_TOKEN = 'Native Injector'
/**
 ...
 */
    let ResolveContext = (function () {
        function ResolveContext(_path) {
        this._path = _path
    }
    /** Gets all the tokens found in the resolve context, de-duplicated */
        ResolveContext.prototype.getTokens = function () {
        return this._path.reduce((acc, node) => {
            return acc.concat(node.resolvables.map(r => {
     return r.token
}))
}, []).reduce(uniqR, [])
    }
    ...
    ResolveContext.prototype.getResolvable = function (token) {
        const matching = this._path.map(node => {
    ...
    ResolveContext.prototype.subContext = function (state) {
        return new ResolveContext(PathFactory.subPath(this._path, node => {
    ...
    ResolveContext.prototype.addResolvables = function (newResolvables, state) {
        ...
        let keys = newResolvables.map(r => {
            return r.token
})
        node.resolvables = node.resolvables.filter(r => {
    ...
    ResolveContext.prototype.resolvePath = function (when, trans) {
        ...
        let matchesPolicy = function (acceptedVals, whenOrAsync) {
        ...
        let promises = this._path.reduce((acc, node) => {
            ...
            let subContext = _this.subContext(node.state)
            let getResult = function (r) {
    ...
    ResolveContext.prototype.getDependencies = function (resolvable) {
        ...
        let subPath = PathFactory.subPath(this._path, x => {
            return x === node
}) || this._path
        let availableResolvables = subPath
            .reduce((acc, node) => {
        ...
        let getDependency = function (token) {
            const matching = availableResolvables.filter(r => {
                return r.token === token
})
 ...
 */
     ...
     */
        ...
        let placeholder = /([:*])([\w\[\]]+)|\{([\w\[\]]+)(?:\:\s*((?:[^{}\\]+|\\.|\{(?:[^{}\\]+|\\.)*\})+))?\}/g,
            ...
            m,
            patterns = []
        let checkParamErrors = function (id) {
...
throw new Error('Duplicate parameter name \'' + id + '\' in pattern \'' + pattern$$1 + '\'')}
        ...
        let matchDetails = function (m, isSearch) {
            // IE[78] returns '' for unmatched groups instead of null
            const id = m[2] || m[3]
            let regexp = isSearch ? m[4] : m[4] || (m[1] === '*' ? '.*' : null)
            let makeRegexpType = function (regexp) {
...
break} // we're into the search part
        ...
        this._compiled = patterns.map(pattern$$1 => {
...
return null}
        ...
        let allParams = this.parameters(),
            pathParams = allParams.filter(param => {
    return !param.isSearch()
}),
            searchParams = allParams.filter(param => {
    return param.isSearch()
}),
            nPathSegments = this._cache.path.map(urlm => {
    return urlm._segments.length - 1
     ...
     return a + x
        ...
        function decodePathArray(string) {
            ...
            let unquoteDashes = function (str) {
...
return this._params}
    ...
    UrlMatcher.prototype.validates = function (params) {
        const _this = this
        let validParamVal = function (param, val$$1) {
            return !param || param.validates(val$$1)
        }
        return pairs(params || {}).map(_a => {
            let key = _a[0],
                val$$1 = _a[1]
...
return null}
        ...
        let pathSegmentsAndParams = urlMatchers.map(UrlMatcher.pathSegmentsAndParams).reduce(unnestR, [])
        // Extract the query params into a separate array
        let queryParams = urlMatchers.map(UrlMatcher.queryParams).reduce(unnestR, [])
        /**
         * Given a Param,
...
return acc + map(encoded, UrlMatcher.encodeDashes).join('-')}
        ...
        let queryString = queryParams.map(param => {
            let _a = getDetails(param),
                squash = _a.squash,
                encoded = _a.encoded,
...
return}
    ...
    UrlMatcher.pathSegmentsAndParams = function (matcher) {
        ...
        let pathParams = matcher._params.filter(p => {
    ...
    UrlMatcher.queryParams = function (matcher) {
        return matcher._params.filter(p => {
    ...
    UrlMatcher.compare = function (a, b) {
        ...
        let aSegments = segments(a),
            bSegments = segments(b)
        ...
        let pairs$$1 = arrayTuples(aSegments.map(weight), bSegments.map(weight))
        // console.table(pairs);
        return pairs$$1.reduce((cmp, weightPair) => {
 ...
 */
    ...
    function ParamTypes() {
        ...
        let makeType = function (definition, name) {
 ...
 return parseInt(val$$1, 10) !== 0 },
        ...
        date: makeDefaultType({
            ...
            decode(val$$1) {
                if (this.is(val$$1))
                    {return val$$1;}
                let match = this.capture.exec(val$$1)
 ...
 */
    let UrlMatcherFactory = (function () {
...
_handler = function (match) {
 return handler.format(match) };
        ...
        function matchPriority(params) {
            const optional = urlMatcher.parameters().filter(param => {
                return param.isOptional
})
            if (!optional.length)                {return 0.000001}
            const matched = optional.filter(param => {
    ...
    UrlRuleFactory.prototype.fromRegExp = function (regexp, handler) {
        ...
        const redirectUrlTo = function (match) {
            ...
            return handler.replace(/\$(\$|\d{1,2})/, (m, what) => {
                return match[what === '$' ? 0 : Number(what)]
            })
        }
        let _handler = isString(handler) ? redirectUrlTo : handler
        let match = function (url) {
    ...
    UrlRuleFactory.isUrlRule = function (obj) {
        return obj && ['type', 'match', 'handler'].every(key => {
 ...
 */
    ...
    defaultRuleSortFn = composeSort(sortBy(pipe(prop('priority'), x => {
    return -x
})), sortBy(pipe(prop('type'), type => {
...
rules.push(this._otherwiseFn)}
    ...
    UrlRouter.prototype.listen = function (enabled) {
        ...
        if (enabled === false) {
            ...
            return this._stopFn = this._stopFn || this._router.urlService.onChange(evt => {
...
return null}
    ...
    UrlRouter.prototype.otherwise = function (handler) {
        ...
        const handlerFn = isFunction(handler) ? handler : val(handler)
 ...
 */
    let eagerResolvePath = function (trans) {
        return new ResolveContext(trans.treeChanges().to)
        .resolvePath('EAGER', trans)
        .then(noop$1)
    }
    let registerEagerResolvePath = function (transitionService) {
 ...
 * See [[StateDeclaration.resolve]]
 */
    let lazyResolveState = function (trans, state) {
        return new ResolveContext(trans.treeChanges().to)
        .subContext(state)
        .resolvePath('LAZY', trans)
        .then(noop$1)
    }
    let registerLazyResolveState = function (transitionService) {
 ...
 */
    let loadEnteringViews = function (transition) {
        const $q = services.$q
        let enteringViews = transition.views('entering')
        if (!enteringViews.length)        {return}
        return $q.all(enteringViews.map(view => {
        return $q.when(view.load())
})).then(noop$1)
    }
    let registerLoadEnteringViews = function (transitionService) {
 ...
 */
    ...
    let registerActivateViews = function (transitionService) {
 ...
 */
    let updateUrl = function (transition) {
    ...
    let registerUpdateUrl = function (transitionService) {
 ...
 */
    let redirectToHook = function (trans) {
...
return $state.target(result.state || trans.to(), result.params || trans.params(), trans.options())}
    ...
    let registerRedirectToHook = function (transitionService) {
 ...
 */
    ...
    let registerOnExitHook = function (transitionService) {
 ...
 */
    let onRetainHook = makeEnterExitRetainHook('onRetain')
    let registerOnRetainHook = function (transitionService) {
 ...
 */
    let onEnterHook = makeEnterExitRetainHook('onEnter')
    let registerOnEnterHook = function (transitionService) {
 ...
 */
    let lazyLoadHook = function (transition) {
    ...
    let registerLazyLoadHook = function (transitionService) {
 ...
 */
    function lazyLoadState(transition, state) {
        const lazyLoadFn = state.lazyLoad
 ...
 * @interalapi
 */
    const TransitionEventType = (function () {
 ...
 */
    let TransitionService = (function () {
    /** @hidden */
    ...
    TransitionService.prototype.dispose = function (router) {
        ...
        values(this._registeredHooks).forEach(hooksArray => {
            return hooksArray.forEach(hook => {
    ...
    TransitionService.prototype._defineDefaultEvents = function () {
        ...
        let TH = TransitionHook
        let paths = this._criteriaPaths
    ...
    TransitionService.prototype._defineDefaultPaths = function () {
        let STATE = exports.TransitionHookScope.STATE,
    ...
    TransitionService.prototype._defineEvent = function (name, hookPhase, hookOrder, criteriaMatchPath, reverseSort, getResultHandler, getErrorHandler, rejectIfSuperseded) {
        ...
        const eventType = new TransitionEventType(name, hookPhase, hookOrder, criteriaMatchPath, reverseSort, getResultHandler, getErrorHandler, rejectIfSuperseded)
 ...
 */
 ...
 return _this._viewConfigs }
    ...
    ViewService.prototype.sync = function () {
        const _this = this
        let uiViewsByFqn = this._uiViews.map(uiv => {
            return [uiv.fqn, uiv]
}).reduce(applyPairs, {})
        /**
...
return false}
     ...
     return vc.$uiViewContextAnchor === (uiViewContext && uiViewContext.name)
        ...
        const depthCompare = curry((depthFn, posNeg, left, right) => {
            return posNeg * (depthFn(left) - depthFn(right))
})
        let matchingConfigPair = function (uiView) {
            const matchingConfigs = _this._viewConfigs.filter(matches(uiView))
        ...
        let configureUIView = function (_a) {
            let uiView = _a[0],
                viewConfig = _a[1]
...
trace.traceViewServiceUIViewEvent('!!!! duplicate uiView named:', uiView)}
    ...
    ViewService.normalizeUIViewTarget = function (context, rawViewName) {
        ...
        if (relativeMatch.exec(uiViewContextAnchor)) {
            const anchor = uiViewContextAnchor.split('.').reduce(((anchor, x) => {
...
name = this.resolvePath(name, base)}
        ...
        if (state && (isStr || (!isStr && (state === stateOrName || state.self === stateOrName)))) {
            ...
            const matches = values(this._states)
                .filter(state => {
                    return new Glob(state.name).matches(name)
})
            if (matches.length > 1) {
                console.log('stateMatcher.find: Found multiple matches for ' + name + ' using glob: ', matches.map(match => {
                    return match.name
}))
            }
            return matches[0]
...
return false}
    ...
    const getUrlBuilder = function ($urlMatcherFactoryProvider, root) {
        ...
        let parsed = parseUrl(stateDec.url),
            parent = state.parent
        let url = !parsed ? stateDec.url : $urlMatcherFactoryProvider.compile(parsed.val, {
            params: state.params || {},
...
throw new Error('Invalid url \'' + url + '\' in state \'' + state + '\'')}
    ...
    let getNavigableBuilder = function (isRoot) {
    return function navigableBuilder(state) {
        return !isRoot(state) && state.url ? state : (state.parent ? state.parent.navigable : null)
    }
}
    let getParamsBuilder = function (paramFactory) {
    return function paramsBuilder(state) {
        ...
        let nonUrlParams = values(mapObj(omit(state.params || {}, urlParams.map(prop('id'))), makeConfigParam))
        return urlParams.concat(nonUrlParams).map(p => {
            return [p.id, p]
}).reduce(applyPairs, {})
    }
}
 ...
 */
    ...
    /** convert resolve: {} and resolvePolicy: {} objects to an array of tuples */
        ...
        return Object.keys(resolveObj || {}).map(token => {
            return ({token, val: resolveObj[token], deps: undefined, policy: resolvePolicies[token]})
})
    }
    ...
    /** true if the object has both `token` and `resolveFn`, and is probably a [[ResolveLiteral]] */
        let isResolveLiteral = function (obj) {
        return Boolean(obj.token && obj.resolveFn)
}
    /** true if the object looks like a provide literal, or a ng2 Provider */
        let isLikeNg2Provider = function (obj) {
        return Boolean((obj.provide || obj.token) && (obj.useValue || obj.useFactory || obj.useExisting || obj.useClass))
}
    /** true if the object looks like a tuple from obj2Tuples */
        let isTupleFromObj = function (obj) {
        return Boolean(obj && obj.val && (isString(obj.val) || isArray(obj.val) || isFunction(obj.val)))
}
    /** extracts the token from a Provider or provide literal */
        let token = function (p) {
        return p.provide || p.token
}
    /** Given a literal resolve or provider object, returns a Resolvable */
    let literal2Resolvable = pattern([
    ...
    // Otherwise, assume it's an object and convert to an Array of tuples
        ...
        let items = isArray(decl) ? decl : objects2Tuples(decl, state.resolvePolicy || {})
...
return null}
        for (const key in builders) {
            ...
            const chain = builders[key].reduce((parentFn, step) => {
...
/** @module state */ /** for typedoc */
/** @internalapi */
    let StateQueueManager = (function () {
 ...
 return config.name }
        ...
        if (states.hasOwnProperty(state.name) || pluck(queue, 'name').indexOf(state.name) !== -1)            {throw new Error('State \'' + state.name + '\' is already defined')}
        queue.push(state)
        this.flush()
        return state
    }
    StateQueueManager.prototype.flush = function () {
        let _a = this,
            ...
            states = _a.states,
            builder = _a.builder
        let registered = [], // states that got registered
            orphans = [], // states that don't yet have a parent registered
            previousQueueLength = {} // keep track of how long the queue when an orphan was first encountered
        while (queue.length > 0) {
            const state = queue.shift()
            let result = builder.build(state)
            let orphanIdx = orphans.indexOf(state)
            if (result) {
                const existingState = this.$registry.get(state.name)
...
orphans.splice(orphanIdx, 1)}
            ...
            const prev = previousQueueLength[state.name]
        ...
        if (registered.length) {
            this.listeners.forEach(listener => {
                return listener('registered', registered.map(s => {
     return s.self
}))
})
        }
 ...
 */ /** for typedoc */
    ...
    StateRegistry.prototype._deregisterTree = function (state) {
        const _this = this
        let all$$1 = this.get().map(s => {
            return s.$$state()
})
        var getChildren = function (states) {
            const children = all$$1.filter(s => {
    ...
    StateRegistry.prototype.deregister = function (stateOrName) {
        ...
        this.listeners.forEach(listener => {
            return listener('deregistered', deregisteredStates.map(s => {
     return s.self
}))
})
 ...
 */
 ...
 return this.router.globals.$current },
    ...
    StateService.prototype._handleInvalidTargetState = function (fromPath, toState) {
        ...
        let checkForRedirect = function (result) {
            if (!(result instanceof TargetState)) {
                return
            }
            let target = result
...
return Rejection.invalid(toState.error()).toPromise()}
            ...
            return callbackResult.then(checkForRedirect).then(result => {
...
throw new Error('Invalid reload state object')}
        ...
        const stateDefinition = reg.matcher.find(identifier, options.relative)
    ...
    StateService.prototype.getCurrentPath = function () {
        const _this = this
        let globals = this.router.globals
        let latestSuccess = globals.successfulTransitions.peekTail()
    ...
    StateService.prototype.transitionTo = function (to, toParams, options) {
        ...
        let ref = this.target(to, toParams, options)
 ...
 */
    let Globals = (function () {
        ...
        let beforeNewTransition = function ($transition$) {
            ...
            let clearCurrentTransition = function () {
 ...
 */ /** */
/** @hidden */
    let makeStub = function (keys) {
        return keys.reduce((acc, key) => {
        return (acc[key] = notImplemented(key), acc)
}, {dispose: noop$1})
    }
    /** @hidden */ let locationServicesFns = ['url', 'path', 'search', 'hash', 'onChange']
    /** @hidden */ let locationConfigFns = ['port', 'protocol', 'host', 'baseHref', 'html5Mode', 'hashPrefix']
    /** @hidden */ let umfFns = ['type', 'caseInsensitive', 'strictMode', 'defaultSquashPolicy']
    /** @hidden */ let rulesFns = ['sort', 'when', 'otherwise', 'rules', 'rule', 'removeRule']
    /** @hidden */ let syncFns = ['deferIntercept', 'listen', 'sync', 'match']
/**
 ...
 */
    ...
    function Transition(fromPath, targetState, router) {
        ...
        let onCreateHooks = this.hookBuilder().buildHooksForPhase(exports.TransitionHookPhase.CREATE)
    ...
    Transition.prototype.applyViewConfigs = function (router) {
        const enteringStates = this._treeChanges.entering.map(node => {
            return node.state
})
    ...
    Transition.prototype.applyRootResolvables = function (router) {
        const _this = this
        let rootResolvables = [
            new Resolvable(UIRouter, () => {
                return router
}, [], undefined, router),
            new Resolvable(Transition, () => {
                return _this
}, [], undefined, this),
            new Resolvable('$transition$', () => {
                return _this
}, [], undefined, this),
            new Resolvable('$stateParams', () => {
                return _this.params()
}, [], undefined, this.params())
        ]
        let rootNode = this._treeChanges.to[0]
    ...
    Transition.prototype.injector = function (state, pathName) {
        ...
        if (state)
            {
path = PathFactory.subPath(path, (node) => {
 return node.state === state || node.state.name === state })}
    ...
    Transition.prototype.addResolvable = function (resolvable, state) {
        ...
        let targetNode = find(topath, node => {
            return node.state.name === stateName
})
        let resolveContext = new ResolveContext(topath)
...
throw new Error('Too many consecutive Transition redirects (20+)')}
        ...
        // ---------------------------------------------------------
        let nodeIsReloading = function (reloadState) {
            return function (node) {
     return reloadState && node.state.includes[reloadState.name]
 }
}
        // Find any "entering" nodes in the redirect path that match the original path and aren't being reloaded
        let matchingEnteringNodes = PathNode.matching(redirectEnteringNodes, originalEnteringNodes)
...
return undefined}
        ...
        const nodeSchemas = tc.to.map(node => {
            return node.paramSchema
})
        let _a = [tc.to, tc.from].map(path => {
                return path.map(x => {
    ...
    Transition.prototype.dynamic = function () {
        ...
        return !changes ? false : changes.map(x => {
            return x.dynamic
}).reduce(anyTrueR, false)
    }
    /**
    ...
    Transition.prototype.run = function () {
        ...
        let onBeforeHooks = hookBuilder.buildHooksForPhase(exports.TransitionHookPhase.BEFORE)
        let syncResult = TransitionHook.runOnBeforeHooks(onBeforeHooks)
        if (Rejection.isTransitionRejectionPromise(syncResult)) {
            syncResult.catch(() => {
                return 0
}) // issue #2676
            let rejectReason = syncResult._transitionRejection
        ...
        const transitionSuccess = function () {
            ...
            let onSuccessHooks = hookBuilder.buildHooksForPhase(exports.TransitionHookPhase.SUCCESS)
            runAllHooks(onSuccessHooks)
        }
        let transitionError = function (reason) {
            trace.traceError(reason, _this)
            _this.success = false
            _this._deferred.reject(reason)
            _this._error = reason
            let onErrorHooks = hookBuilder.buildHooksForPhase(exports.TransitionHookPhase.ERROR)
            runAllHooks(onErrorHooks)
        }
        trace.traceTransitionStart(this)
        // Chain the next hook off the previous
        let appendHookToChain = function (prev, nextHook) {
            return prev.then(() => {
                return nextHook.invokeHook()
})
        }
        // Run the hooks, then resolve or reject the overall deferred in the .then() handler
        let asyncHooks = hookBuilder.buildHooksForPhase(exports.TransitionHookPhase.ASYNC)
...
return 'Param values not valid for state \'' + state.name + '\'';
    ...
    Transition.prototype.toString = function () {
        ...
        let avoidEmptyHash = function (params) {
...
return '[circular ref]'}
        ...
        return JSON.stringify(o, (key, val$$1) => {
        return format(val$$1)
}).replace(/\\"/g, '"')
    }
 ...
 * @module core
 */ /** for typedoc */
    const UIRouterPluginBase = (function () {
 ...
 */ /** */
    ...
    let ng = (ng_from_import && ng_from_import.module) ? ng_from_import : ng_from_global
...
config = {component: config}}
    ...
    Ng1ViewConfig.prototype.load = function () {
        ...
        let params = this.path.reduce((acc, node) => {
            return extend(acc, node.paramValues)
}, {})
        let promises = {
 ...
 */
    ...
    TemplateFactory.prototype.fromConfig = function (config, params, context) {
        ...
        let asTemplate = function (result) {
            return services.$q.when(result).then(str => {
     return ({template: str})
})
}
        let asComponent = function (result) {
            return services.$q.when(result).then(str => {
...
url = url(params)}
    ...
    TemplateFactory.prototype.makeComponentTemplate = function (uiView, context, component, bindings) {
        bindings = bindings || {}
        // Bind once prefix
        let prefix = ng.version.minor >= 3 ? '::' : ''
        let attributeTpl = function (input) {
            let name = input.name,
                type = input.type
 ...
 */
    let getStateHookBuilder = function (hookName) {
 ...
 */
    let Ng1LocationServices = (function () {
...
this.$location.url(newUrl)}
    ...
    Ng1LocationServices.prototype._runtimeServices = function ($rootScope, $location, $sniffer, $browser) {
        ...
        $rootScope.$on('$locationChangeSuccess', evt => {
            return _this._urlListeners.forEach(fn => {
     return fn(evt)
})
})
    ...
    Ng1LocationServices.monkeyPatchPathParameterType = function (router) {
        ...
        pathType.encode = function (val$$1) {
            return val$$1 != null ? val$$1.toString().replace(/(~|\/)/g, m => {
                return ({'~': '~~', '/': '~2F'}[m])
}) : val$$1
        }
        pathType.decode = function (val$$1) {
            return val$$1 != null ? val$$1.toString().replace(/(~~|~2F)/g, m => {
                return ({'~~': '~', '~2F': '/'}[m])
}) : val$$1
        }
    }
 ...
 */
    let UrlRouterProvider = (function () {
    /** @hidden */
...
/** for typedoc */
    ...
    let mod_init = ng.module('ui.router.init', [])
    let mod_util = ng.module('ui.router.util', ['ng', 'ui.router.init'])
    let mod_rtr = ng.module('ui.router.router', ['ui.router.util'])
    let mod_state = ng.module('ui.router.state', ['ui.router.router', 'ui.router.util', 'ui.router.angular1'])
    let mod_main = ng.module('ui.router', ['ui.router.init', 'ui.router.state', 'ui.router.angular1'])
    let mod_cmpt = ng.module('ui.router.compat', ['ui.router'])
    let router = null
...
// $urlRouter service and $urlRouterProvider
    const getUrlRouterProvider = function (router) {
        return router.urlRouterProvider = new UrlRouterProvider(router)
    }
// $state service and $stateProvider
// $urlRouter service and $urlRouterProvider
    let getStateProvider = function () {
...
/** @hidden */
    function stateContext(el) {
        ...
        let path = parse('$cfg.path')($uiView)
 ...
 */
 ...
 return processedDef($state, element, rawDef) };
                ...
                if (ref.paramExpr) {
                    scope.$watch(ref.paramExpr, (val$$1) => {
 ...
 */
 ...
 return processedDef($state, element, rawDef) };
                ...
                var watchDeregFns = inputAttrs.reduce((acc, attr) => {
            ...
            controller: ['$scope', '$element', '$attrs',
                function ($scope, $element, $attrs) {
                    ...
                    function update() {
                        ...
                        let getClasses = function (stateList) {
                            return stateList.map(x => {
                        ...
                        let fuzzyClasses = getClasses(states.filter(x => {
                            return $state.includes(x.state.name, x.params)
}))
                        let exactlyMatchesAny = Boolean(states.filter((x) => {
 return $state.is(x.state.name, x.params) }).length)
                        ...
                        let removeClasses = allClasses.filter(cls => {
...
name = $interpolate(attrs.uiView || attrs.name || '')(scope) || '$default'
                    ...
                    function cleanupLastView() {
                        ...
                        if (currentEl) {
                            let _viewData_1 = currentEl.data('$uiViewAnim')
...
animLeave = $q.defer()
                        ...
                        var cloned = $transclude(newScope, (clone) => {
...
/** @hidden */
    ...
    let getResolveAs = parse('viewDecl.resolveAs')
    return {
        restrict: 'ECA',
        priority: -400,
        compile(tElement) {
            ...
            return function (scope, $element) {
                ...
                let cfg = data.$cfg || {viewDecl: {}, getTemplate: ng_from_import.noop}
                ...
                if (controller) {
                    let controllerInstance = $controller(controller, extend({}, locals, {$scope: scope, $element}))
                    if (controllerAs) {
                ...
                if (isString(cfg.viewDecl.component)) {
                    let cmp_1 = cfg.viewDecl.component
...
/** @hidden TODO: move these callbacks to $view and/or `/hooks/components.ts` or something */
    ...
    let hookOptions = {bind: controllerInstance}
    // Add component-level hook for onParamsChange
    if (isFunction(controllerInstance.uiOnParamsChanged)) {
        const resolveContext = new ResolveContext(cfg.path)
        let viewCreationTrans_1 = resolveContext.getResolvable('$transition$').data
        // Fire callback on any successful transition
        let paramsUpdated = function ($transition$) {
            // Exit early if the $transition$ is the same as the view was created within.
            // Exit early if the $transition$ will exit the state the view is for.
            if ($transition$ === viewCreationTrans_1 || $transition$.exiting().indexOf(viewState) !== -1)                {return}
            const toParams = $transition$.params('to')
            let fromParams = $transition$.params('from')
            let toSchema = $transition$.treeChanges().to.map(node => {
                return node.paramSchema
}).reduce(unnestR, [])
            let fromSchema = $transition$.treeChanges().from.map(node => {
                return node.paramSchema
}).reduce(unnestR, [])
            // Find the to params that have different values than the from params
            let changedToParams = toSchema.filter(param => {
                const idx = fromSchema.indexOf(param)
                return idx === -1 || !fromSchema[idx].type.equals(toParams[param.id], fromParams[param.id])
            })
            // Only trigger callback if a to param has changed or is new
            if (changedToParams.length) {
                const changedKeys_1 = changedToParams.map(x => {
                    return x.id
})
                // Filter the params to only changed/new to params.  `$transition$.params()` may be used to get all params.
                let newValues = filter(toParams, (val$$1, key) => {
